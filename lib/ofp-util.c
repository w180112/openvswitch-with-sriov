...
void send_netlink(uint32_t type, uint8_t vf[], uint16_t vlan, char mac[]);
...
void send_netlink(uint32_t type, uint8_t vf[], uint16_t vlan, char mac[])
{
    ...
    netmsg.type = type;
    memcpy(netmsg.vf,vf,64);
    if (type == 0x0d) //0x0d for vlan
	netmsg.vlan = vlan;
    else if (type == 0x0e)
	memcpy(netmsg.mac,mac,sizeof(mac));
    ...
    nlh->nlmsg_flags = 0;
    memcpy(NLMSG_DATA(nlh),&netmsg,sizeof(netmsg));
                        ...
                        ofp_port_t max_port, uint8_t max_table)
{
    ...
    if (raw == OFPRAW_OFPT11_FLOW_MOD) {
        ...
        error = ofputil_pull_ofp11_match(&b, tun_table, vl_mff_map, &fm->match,
                                         NULL);
	//FILE *fp = fopen("/root/test.txt","a");
	//fprintf(fp, "lib/ofp-util.c/flowmod metadata = %lu\n", fm->match.flow.metadata);
	/*for(int i=0; i<4; i++) {
                fprintf(fp, "lib/ofp-util.c/flowmod pad1 = %u\n", fm->match.flow.pad1[i]);
        }*/
	fprintf(fp, "lib/ofp-util.c/flowmod vlan = %u\n", fm->match.flow.vlan_tci);
        fprintf(fp, "lib/ofp-util.c/flowmod match = %u, size of eth type = %u\n", fm->match.flow.dl_type, sizeof(fm->match.flow.dl_type));
	fprintf(fp, "lib/ofp-util.c/flowmod field = %lu\n", fm->match.flow.mf_field);
	for(int i=0; i<6; i++) {
	        fprintf(fp, "lib/ofp-util.c/flowmod eth src = %u\n", fm->match.flow.dl_src.ea[i]);
	}
	for(int i=0; i<3; i++) {
                fprintf(fp, "lib/ofp-util.c/flowmod eth src = %u\n", fm->match.flow.dl_src.be16[i]);
	}
	for(int i=0; i<6; i++) {
                fprintf(fp, "lib/ofp-util.c/flowmod eth dst = %u\n", fm->match.flow.dl_dst.ea[i]);
        }
        for(int i=0; i<3; i++) {
                fprintf(fp, "lib/ofp-util.c/flowmod eth dst = %u\n", fm->match.flow.dl_dst.be16[i]);
	}
    ...
    fprintf(fp, "lib/ofp-util.c/decode flowmod port = %u cmd = %u ofpacts = %u %u %u out port = %u in port = %lu\n", fm->out_port, fm->command, fm->ofpacts->type, fm->ofpacts->raw, fm->ofpacts->len, ofp_to_u16(ofpact_get_OUTPUT(fm->ofpacts)->port), fm->match.flow.in_port.ofp_port);
    /* init vf list */
    uint8_t vf[64] = {0};
    /*for(int i=0; i<(ofpacts->size)/16; i++) {
	//each action is 16 bytes. first action is at data of opfbuf +4 addr.
	void *tmp = ofp;
	vf[i] = *(uint8_t *)((ofpacts->data)+4+16*i);
	fprintf(fp, "port = %d\n", vf[i]);
    }
    vf[0] = *(uint8_t *)((ofpacts->data)+4);
    vf[1] = *(uint8_t *)((ofpacts->data)+20);
    vf[2] = *(uint8_t *)((ofpacts->data)+36);
	fprintf(fp, "port = %d", vf[0]);
	fprintf(fp, "port = %d\n", vf[1]);*/

    if (fm->match.flow.in_port.ofp_port == 3) {
	for(int i=0; i<(ofpacts->size)/16; i++) {
        //each action is 16 bytes. first action is at data of opfbuf +4 addr.
            vf[i] = *(uint8_t *)((ofpacts->data)+4+16*i);
            fprintf(fp, "port = %d\n", vf[i]);
        }
	//struct ofpact_output *tmp_act = ofpact_get_OUTPUT(fm->ofpacts);
	//struct ofpact tmp_ofpact = tmp_act->ofpact;
	//fprintf(fp, "size = %u allocated = %u base = %x data = %x header = %x msg = %x\n", ofpacts->size, ofpacts->allocated, ofpacts->base, ofpacts->data, ofpacts->header, ofpacts->msg);
	//fprintf(fp, "recv sfc %u %u\n", ofp_to_u16(tmp_act->port), ofp_to_u16(ofpact_get_OUTPUT(&tmp_ofpact)->port));	
	if (sfc_proc(fp,vf) > 0) 
	...
	//send_netlink(ofp_to_u16(ofpact_get_OUTPUT(fm->ofpacts)->port),fm->match.flow.vlan_tci/256);
	unsigned char mac[12] = {0x0,0x0,0x0,0x0,0x0,0x0};
	send_netlink(0x0d,vf,fm->match.flow.vlan_tci/256,mac);
    }
    /*if (fm->match.flow.dl_type == 52360) {
	send_netlink(3,1);
    }
    else if (fm->match.flow.dl_type == 1544) {
        send_netlink(3,0);
    }*/
    fclose(fp);
                          ...
                          struct ofpbuf *ofpacts)
{
	...
	FILE *fp = fopen("/root/test.txt","a");
	for(int i=0; i<7; i++) {
    	    fprintf(fp, "lib/ofp-util.c/packetout payload = %u\n", opo->pad[i]);
	}
     	fclose(fp);	
